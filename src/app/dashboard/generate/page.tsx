...
import { selectBestTemplate, PREMIUM_TEMPLATES } from "@/lib/templates/premium-templates";
...
export default function GeneratePage() {
  ...
  const analyzeAndSuggestCharts = async (
    ...
    // First, try to find premium template match
    const dataForTemplateSelection = labels.map((label, i) => ({
      label,
      value: values[i],
      month: /jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec/i.test(label) ? label : undefined,
      date: /^\d{4}-\d{2}-\d{2}/.test(label) ? label : undefined,
      name: label, // For ROAS template
    }));

    const premiumMatch = selectBestTemplate(dataForTemplateSelection);
    if (premiumMatch) {
      // Create a virtual template for premium template
      const virtualTemplate: Template = {
        id: premiumMatch.id,
        name: premiumMatch.name,
        description: premiumMatch.description,
        chart_type: premiumMatch.category,
        is_premium: premiumMatch.isPremium,
        example_data: null,
        thumbnail_url: null,
      };
      suggestions.push({
        ...
        confidence: 95,
        reason: `Premium ${premiumMatch.name} template - perfect match for your data`,
      });
    }
